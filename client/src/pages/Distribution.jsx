...
function Media({ fileId }) {
  ...
  const sx = { width: "100%", height: 200, objectFit: "cover", borderRadius: 2, bgcolor: "action.hover", display: "block" };
  ...
  return video
    ? <Box component="video" src={src} controls sx={{ ...sx, objectFit: "contain", bgcolor: "#000" }} />
    ...
    <Card sx={{ height: "100%" }}>
      <CardContent>
        <Stack spacing={1.5}>
          ...
          <Media fileId={fileId} />
          <Button component="label" variant="outlined" startIcon={<UploadIcon />} disabled={uploading} size="small">
            {uploading ? "Enviando..." : fileId ? "Trocar foto/vídeo" : "Subir foto/vídeo"}
...
export default function Distribution() {
  ...
  return (
    <>
      ...
      {!stage && !loading ? (
        <EmptyState message="Crie uma etapa chamada 'Programação' no quadro de Tarefas para usar a Distribuição." />
      ) : loading ? (
        <Box sx={{ display: "grid", placeItems: "center", py: 6 }}><CircularProgress /></Box>
      ) : items.length === 0 ? (
        <EmptyState message="Nenhuma peça na Programação ainda. Mova as tarefas prontas para a coluna 'Programação'." />
      ) : (
