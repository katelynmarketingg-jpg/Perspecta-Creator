...
router.post("/", (req, res) => {
  ...
  const info = db
    ...
    .run(b.name, Number(b.default_price) || 0, b.contract_template ?? null, normalizeItems(b.items_schema), b.category ?? null, b.contract_style ?? null, req.orgId);
...
router.put("/:id", (req, res) => {
  ...
  db.prepare("UPDATE services SET name = ?, default_price = ?, contract_template = ?, items_schema = ?, category = ?, contract_style = ? WHERE id = ? AND org_id = ?").run(
    ...
    b.contract_style !== undefined ? b.contract_style : cur.contract_style,
    req.params.id,
    req.orgId
  );
