...
router.post("/", (req, res) => {
  ...
  const info = db
    .prepare("INSERT INTO services (name, default_price, contract_template, org_id) VALUES (?, ?, ?, ?)")
    .run(b.name, Number(b.default_price) || 0, b.contract_template ?? null, req.orgId);
...
router.put("/:id", (req, res) => {
  ...
  const b = req.body || {};
  db.prepare("UPDATE services SET name = ?, default_price = ?, contract_template = ? WHERE id = ? AND org_id = ?").run(
    b.name ?? cur.name,
